...
% ── Abstract ───────────────────────────────────────────────────────
\begin{abstract}
Security is still treated as a final gate in most software delivery pipelines,
checked just before deployment rather than throughout development.
This paper presents \emph{SecureShift}, a fully automated DevSecOps pipeline
that integrates four scanning disciplines into a single GitHub Actions
CI/CD workflow: dependency auditing, static application security testing
(SAST), container image scanning, and dynamic application security testing
(DAST).
...
are addressed.
The four-tool combination surfaces all thirteen vulnerabilities with one
YAML workflow file and one API token, showing that shift-left security
is practical for teams without dedicated security staff.
\end{abstract}
...
Adoption is growing, but most teams still hit the same barriers.
Security tooling is fragmented across vendors, output formats are inconsistent,
and reading scanner results requires expertise that small engineering teams
rarely have.
We address those barriers with a reference implementation deployable from
...
\begin{enumerate}
  ...
  \item A remediated version of VulnAPI (``app-fixed'') that demonstrates
        the end-to-end workflow from detection to resolution.
\end{enumerate}
...
After the scan, the job queries the SonarCloud Issues REST API
(\texttt{/api/issues/search}) and writes all open, unresolved findings to
a JSON file.
This two-phase design (scan then export) separates issue collection from the
SonarCloud web UI, so the dashboard shows findings without a separate login.
...
The tarball is uploaded as a short-lived artifact (1-day retention) and
shared with the Trivy and DAST jobs, so both scan the exact image that
...
The tools cover different ground.
No single tool detects all thirteen vulnerabilities, which confirms the
value of running all four in parallel.
...
No single scanning tool is sufficient on its own.
...
other tools.
The coverage difference across tools justifies the added runtime.

\subsection{Developer Experience}

Setup is minimal: the only external credential required is a \texttt{SONAR\_TOKEN}
added to the repository secrets.
...
SecureShift shows that four-tool security scanning costs nothing beyond
GitHub's free tier and one API token.
The full implementation is open source at
\texttt{github.com/neelp03/CMPE272-project}.
